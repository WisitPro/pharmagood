    ...
    public function PharmacistTask($data)
{
    ...
    if (!empty($day1) && !empty($day2)) {
        if ($day1 == $day2) {
            $conditions[] = "DATE(r.req_modify) = '$day1'";
        } else {
            $conditions[] = "DATE(r.req_modify) BETWEEN '$day1+00:00:00' AND '$day2+23:59:59'";
        }
    } elseif (!empty($day1)) {
        $conditions[] = "DATE(r.req_modify) = '$day1'";
    } elseif (!empty($day2)) {
        $conditions[] = "DATE(r.req_modify) = '$day2'";
    }
...
public function OrderReport($data)
{
    ...
    $sql = "SELECT * FROM tbl_payprove p RIGHT JOIN tbl_order o ON p.order_id = o.order_id
    LEFT JOIN tbl_customer c ON c.cus_id = o.cus_id LEFT JOIN tbl_admin a ON p.adm_id = a.adm_id WHERE ";
...
public function OrderlistReport($data)
{
    ...
    $sql = "SELECT * FROM tbl_orderlist ol LEFT JOIN tbl_order o ON o.order_id = ol.order_id 
